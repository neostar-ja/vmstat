#!/bin/bash
# Initialize RBAC System - Creates tables, roles, permissions, and 3 test users
# This script runs the SQL directly against PostgreSQL

# Load environment variables
source /opt/code/sangfor_scp/.env 2>/dev/null || true

# Database connection settings from environment
DB_HOST="${pgSQL_HOST:-localhost}"
DB_PORT="${pgSQL_HOST_PORT:-5432}"
DB_NAME="${pgSQL_DBNAME:-sangfor_scp}"
DB_USER="${pgSQL_USERNAME:-postgres}"
DB_PASS="${pgSQL_PASSWORD}"

echo "============================================================"
echo "🚀 RBAC System Initialization"
echo "============================================================"
...
-- Update role_id for existing users based on role column
...
SELECT 
    'viewer_user',
    'viewer@vmstat.local',
    '$2b$12$xqOmDIiIzVmPJnKLQv3c1yqBwHLPMFTQjhOXeqOmDIiIzVmPJnK3',
    'Report Viewer',
    'viewer',
...
DECLARE
    ...
    user_count INT;
BEGIN
    SELECT COUNT(*) INTO role_count FROM webapp.roles;
    SELECT COUNT(*) INTO perm_count FROM webapp.permissions;
    SELECT COUNT(*) INTO user_count FROM webapp.users WHERE is_active = TRUE;
    
    RAISE NOTICE '';
    RAISE NOTICE '============================================================';
    RAISE NOTICE '✅ RBAC System Initialized Successfully';
    RAISE NOTICE '============================================================';
    ...
    RAISE NOTICE '   admin_user / Admin@2026! (Admin)';
    RAISE NOTICE '   manager_user / Manager@2026! (Manager)';
    RAISE NOTICE '   viewer_user / Viewer@2026! (Viewer)';
    RAISE NOTICE '============================================================';
END $$;
...
echo "   Username: admin_user    Password: Admin@2026!    Role: admin"
echo "   Username: manager_user  Password: Manager@2026!  Role: manager"
echo "   Username: viewer_user   Password: Viewer@2026!   Role: viewer"
